...
export class RecetaComponent implements OnInit {
  ...
  trackId(index: number, item: IReceta): number {
    return item.id!;
  }
  ...
  buscarRecetas(page?: number, dontNavigate?: boolean): void {
    this.isLoading = true;
    const pageToLoad: number = page ?? this.page ?? 1;

    if (this.filtro !== '') {
      this.recetaService
        .busquedaRecetas(this.filtro, {
          page: pageToLoad - 1,
          size: this.itemsPerPage,
          sort: this.sort(),
        })
  ...
  protected onSuccess(data: IReceta[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
