...
/**
 * REST controller for managing {@link com.labdentalist.app.domain.Cliente}.
 */
@RestController
@RequestMapping("/api")
public class ClienteResource {
    ...
    @GetMapping("/clientes/busqueda-clientes")
    public ResponseEntity<List<ClienteDTO>> busquedaCliente(
        Pageable pageable,
        @RequestParam(required = false, defaultValue = "") String filtro
    ) {
        filtro = !filtro.equals("undefined") ? filtro : "%";
        Page<ClienteDTO> page = clienteService.busquedaCliente(filtro, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
}
